...
- Algunos módulos de test quedaron huérfanos tras el reemplazo de los providers directos por el gateway LiteLLM (`test_anthropic_provider.py`, `test_gemini*.py`, `test_llmclient.py`, `test_openai_provider.py` — importan `app.llm.providers`, movido a `app.llm.legacy`) y no corren; pendiente decidir si se actualizan o se borran.
...
- **Tests huérfanos** (`test_anthropic_provider.py`, `test_gemini*.py`, `test_llmclient.py`, `test_openai_provider.py`) referenciando el paquete `app.llm.providers` ya movido a `app.llm.legacy` — no corren, pendiente actualizarlos o borrarlos.
